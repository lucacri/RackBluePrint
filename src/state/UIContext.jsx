...
export function ModalProvider({ children }) {
  ...
  const openDevModal  = useCallback((prefill, mode = 'lib') => setDevModal({ prefill: prefill ?? null, mode }), [])
  const closeDevModal = useCallback(() => setDevModal(null), [])

  const openConfirm  = useCallback((msg, cb) => setConfirmModal({ msg, cb }), [])
  ...
  return (
    <ModalCtx.Provider value={{
      devModal, openDevModal, closeDevModal,
      confirmModal, openConfirm, closeConfirm, doConfirm,
    }}>
      {children}
    </ModalCtx.Provider>
  )
...
export function ToastProvider({ children }) {
  ...
  return (
    <ToastCtx.Provider value={toast}>
      {children}
      {msg && (
        <div className="toast show" aria-live="polite">{msg}</div>
      )}
    </ToastCtx.Provider>
  )
}
