...
import { findRow, normalizeState } from "../state/helpers";
...
export default function Toolbar() {
	...
	function handleSizeChange(e) {
		...
		const doResize = () => {
			atRisk.forEach((d) =>
				dispatch({ type: "DELETE_DEVICE", deviceId: d.id }),
			);
	...
	function handleClear() {
		...
		openConfirm("Remove all devices from the rack?", () =>
	...
	function handleExport() {
		const filename =
			(state.rackName || "rack").replace(/[^a-z0-9_-]/gi, "_") + ".json";
	...
	return (
		<header className="toolbar">
			...
			<input
				...
				autoComplete="off"
				onChange={(e) =>
					dispatch({ type: "SET_RACK_NAME", name: e.target.value })
				}
			/>
			...
			<button
				...
				title="Import a saved .json layout"
			>
				↑ Import
			</button>
			<button
