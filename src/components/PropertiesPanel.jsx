...
import { CAT_COLORS, CAT_NAMES, PORT_LBL } from "../data/constants";
...
function RackSummary() {
	...
	return (
		<div className="props-body">
			...
			<div className="props-section-title" style={{ marginTop: 16 }}>
				Power
			</div>
			<div className="props-summary-row">
			...
			<div className="props-summary-row">
				...
				<span className="props-value">
					<input
						...
						onChange={(e) =>
							dispatch({ type: "SET_POWER_CAP", cap: +e.target.value })
						}
					/>
					W
				</span>
						...
						{Object.values(state.devices).map((dev) => {
							...
							return (
								<div
									...
									onClick={() =>
										dispatch({ type: "SELECT_DEVICE", deviceId: dev.id })
									}
								>
			...
			{Object.keys(state.devices).length === 0 && (
				<div className="props-empty">
					Drag devices from the library onto the rack to get started.
				</div>
			)}
		</div>
	);
...
function DeviceDetail({ device }) {
	...
	function handleDelete() {
		openConfirm(`Remove "${device.name}" from the rack?`, () =>
	...
	return (
		<div className="props-body">
			...
			<div className="props-dev-header" style={{ background: bg, color: fg }}>
				<span className="props-dev-header-cat">
					{CAT_NAMES[device.cat] ?? device.cat}
				</span>
				...
				<div className="props-cat-grid">
					{Object.entries(CAT_COLORS).map(([cat, { bg: cbg, fg: cfg }]) => (
						<button
							key={cat}
							className={
								"props-cat-btn" + (device.cat === cat ? " is-selected" : "")
							}
							style={
								device.cat === cat
									? { background: cbg, color: cfg, borderColor: cfg }
									: {}
							}
							onClick={() => set({ cat })}
				...
				<div className="props-pwr-grid">
					{PWR_TYPES.map((pt) => (
						<button
							key={pt}
							className={
								"props-pwr-btn" + (pwrType === pt ? " is-selected" : "")
							}
				...
				{pwrType !== "passive" && (
					<div className="props-pwr-row">
						<input
							...
							style={{ width: 72 }}
						/>
						<span className="props-label" style={{ alignSelf: "center" }}>
							W
						</span>
						<input
							className="props-field-input"
			...
			<div className="props-field">
				...
				{(!device.ports || device.ports.length === 0) && (
					<div className="props-muted" style={{ fontSize: 11 }}>
						No ports defined.
					</div>
				)}
...
export default function PropertiesPanel() {
	...
	return (
		<aside className="panel-right">
			<div className="panel-head">{selDev ? "Properties" : "Rack info"}</div>
