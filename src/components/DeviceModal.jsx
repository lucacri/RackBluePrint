...
export default function DeviceModal() {
	...
	function updatePort(key, field, value) {
		setForm((f) => ({
			...f,
			ports: f.ports.map((p) =>
				p._key === key ? { ...p, [field]: field === "n" ? +value : value } : p,
			),
		}));
	}
		...
		>
			...
			>
				...
				<div className="modal-body">
							...
							{CAT_ORDER.map((cat) => {
								...
								return (
									<button
										...
										style={
											sel ? { background: bg, color: fg, borderColor: fg } : {}
										}
										onClick={() => set("cat", cat)}
									>
										{CAT_NAMES[cat] ?? cat}
					...
					<div className="form-row">
						<div className="ports-header">
							...
							<button type="button" className="btn btn-sm" onClick={addPort}>
								+ Add port
							</button>
						</div>
				...
				<div className="modal-footer">
					<button type="button" className="btn" onClick={closeDevModal}>
						Cancel
					</button>
